# P0.3 — Construire une Narration Forecast pour le COMEX  
NOVAFOOD GLOBAL — Communication Exécutive & Data Storytelling

---

## 🎯 Objectif du cas

Construire une narration exécutive (1 page) destinée au COMEX NOVAFOOD France pour :

- présenter la tendance prévisionnelle,  
- mettre en avant les drivers clés,  
- expliquer les risques & opportunités,  
- préparer les décisions stratégiques.

---

# 1. Contexte

Période analysée : T3 & T4  
Gamme : NutriBox + FreshBite  
Pays : France + Benelux

Problématiques :

- chute distribution FreshBite Benelux (-12%)  
- inflation matières premières  
- succès NutriBox Vegan  
- météo instable (impact NutriBox Ice)  
- tension supply Vietnam  
- croissance e-commerce France (+18%)  

---

# 2. Travail demandé

---

## 🔵 Étape 1 — Construire la narration en 5 blocs

1. **Situation actuelle (Actuals YTD)**  
2. **Tendance prévisionnelle (Next 6 months)**  
3. **Drivers positifs**  
4. **Risques / alertes**  
5. **Décisions recommandées**

---

## 🔵 Étape 2 — Ajouter les éléments clés attendus par un COMEX

- 3 messages prioritaires  
- 3 chiffres majeurs  
- 1 décision immédiate  
- 1 recommandation stratégique  
- 1 alerte supply  

---

## 🔵 Étape 3 — Créer la version finale COMEX (10–12 lignes)

Style attendu :

- clair  
- factuel & chiffré  
- orienté décision  
- sans détails techniques  
- ton exécutif  

---

# 3. Livrables attendus

- Narration forecast (1 page)  
- Tableau “Drivers / Risques”  
- Synthèse KPI  
- Slide COMEX  

---

# 4. Critères d’évaluation

✔ Capacité à raconter les données  
✔ Impact des messages  
✔ Clarté & concision  
✔ Pertinence des décisions proposées  

---
